python3 trainer.py --load_model_init "RWKV-4-Pile-430M-20220808-8066.pth" \
--data_file train.npy --experiment_dataloader --vocab_size 50277 --vocab_size_delta 1 \
--ctx_len 1024 --batch_size 2 \
--n_layer 24 --n_embd 1024 --head_qk 0 \
--lr_init 1e-5 --lr_final 1e-5 --warmup_steps 0 --beta1 0.9 --beta2 0.999 --adam_eps 1e-8 \
--accelerator gpu --devices 1 --precision 16 --strategy deepspeed_stage_3_offload \
--allgather_bucket_size 100 --reduce_bucket_size 100 \
--log_every_n_steps 5 --default_root_dir /d_drive/rwkv/ --val_check_interval 100 --limit_val_batches 300
